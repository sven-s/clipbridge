...
- Files stored unencrypted in `~/.clipsync/slots/`
...
| Local attacker reading config                   | `~/.clipsync/config.json` is mode `0600`                     |
| Local attacker reading in-flight files          | `~/.clipsync/slots/*` are mode `0600` but unencrypted        |
...
- **Authorization revocation** — if you suspect the secret leaked, regenerate it manually by deleting `~/.clipsync/config.json` and restarting the app. There's no per-machine credentialing.
...
2. Stored at `~/.clipsync/config.json` mode `0600`
